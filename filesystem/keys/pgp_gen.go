...
import (
	"DetectAlg/cryptography/decryption"
	"DetectAlg/cryptography/encryption"
	"crypto/ecdsa"
	"crypto/elliptic"
	"crypto/rand"
	"crypto/rsa"
...
func (pgpKeysGen *PgpKeysGen) GenerateEcPgpKeys(req RequestData) (ReturnType, error) {
	...
	data := encryption.DataToEncrypt{
		Data:       string(privKeyPEM),
		Passphrase: req.Passphrase,
	}

	encryptedPrivKey, err := encryption.AES256Encryption(data)
	if err != nil {
...
func (pgpKeysGen *PgpKeysGen) DecryptPgpPrivKey(passphrase string, keyFolderPath string) (string, error) {
	...
	decryptedPrivKeyPEM, err := decryption.AES256Decryption(decryption.DataToDecrypt{
